...
if (count($arrImg)) {
	...
	echo '<img src="'. $objItem->getImgUrl('filename','',2).'" border="0" alt="'.$objItem->get('title').'" class="lft"/>';
}
...
if (count($arrImg)) {
	...
	foreach($arrImg as $objItem) {
		echo '<div style="float:left; padding: 10px 10px 10px 0px"><img src="'
			.$objItem->getImgUrl('filename','',2).'" border="0" alt="'.$objItem->get('title').'" /></div>';
	}
	echo '</div>';
}
