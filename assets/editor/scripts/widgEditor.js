...
var widgStylesheet = "../assets/editor/css/widgContent.css";
...
widgSelectBlockOptions.push("", "- Change style -");
...
function widgInit(full)
{
	widgToolbarItems.push("bold");
	widgToolbarItems.push("italic");
	widgToolbarItems.push("justifyleft");
	widgToolbarItems.push("justifycenter");
	widgToolbarItems.push("justifyright");
...
function widgEditor(replacedTextareaID)
{
	...
	this.theIframe.style.height = this.theTextarea.style.height;
...
widgEditor.prototype.cleanSource = function()
{
	...
	theHTML = theHTML.replace(/<br \/>\s*<\/(h1|h2|h3|h4|h5|h6|li|p|th|td)/g, "</$1");
	
	/* Replace improper IMGs */
	theHTML = theHTML.replace(/(<img [^>]+[^\/])>/g, "$1 />");
	
	/* secure empty TDs */
	theHTML = theHTML.replace(/(<t[d|h]{1}[^>]*>)\s*(<\/t[d|h]{1}>)/g, "$1&nbsp;$2");
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "justifyleft":
				this.addButton(this.theList.id + "ButtonJustifyleft", "widgButtonJustifyleft", "Align left", "justifyleft");
				break;
				
			case "justifycenter":
				this.addButton(this.theList.id + "ButtonJustifycenter", "widgButtonJustifycenter", "Align center", "justifycenter");
				break;
				
			case "justifyright":
				this.addButton(this.theList.id + "ButtonJustifyright", "widgButtonJustifyright", "Align right", "justifyright");
				break;
...
function widgToolbarAction()
{
	...
	switch (this.action)
	{
		...
		case "image":
			...
			//var theImg = parseInt(prompt("Enter the reference of this image:", ""));
			
			//$('cms_loading').setStyle('display','block');
			$('tzneditor_pop_img').setStyle('display','block');
			
			$('edt_img_sel').empty();
			var arrTmp = $$('span.imgname');
			console.log('we have '+arrTmp.length+' image(s)');
			for(i=1; i<=arrTmp.length;i++) {
				if (!arrTmp[i-1].get('text')) continue;
				e = new Element('option',{'value':i});
				e.appendText(arrTmp[i-1].innerHTML);
				e.injectInside('edt_img_sel');
			}
			
			theWidgCurrent = theWidgEditor;
			break;
		...
		default:
			...
			if (this.action == "justifyleft")
			{
				theAction = "Justifyleft";
				theWidgEditor.theToolbar.setState("Justifycenter", "off");
				theWidgEditor.theToolbar.setState("Justifyright", "off");
			}
			
			if (this.action == "justifycenter")
			{
				theAction = "Justifycenter";
				theWidgEditor.theToolbar.setState("Justifyleft", "off");
				theWidgEditor.theToolbar.setState("Justifyright", "off");
			}
			
			if (this.action == "justifyright")
			{
				theAction = "Justifyright";
				theWidgEditor.theToolbar.setState("Justifyleft", "off");
				theWidgEditor.theToolbar.setState("Justifycenter", "off");
			}
...
function widgImgAdd() {
	var theWidgEditor = theWidgCurrent;
	theWidgCurrent = 0;
	var theIframe = theWidgEditor.theIframe;
	var theSelection = "";
	
	var arrTmp = $$('span.imgname');
	var theImg = $('edt_img_sel').get('value');
	
	console.log('theImg:'+theImg+', arrTmp:'+arrTmp.length);
	
	if (theImg > 0 && theImg <= arrTmp.length)
	{
		var theAlign = $('edt_img_alg').get('value');
		var theAlt = $('edt_img_alt').get('value');
		var theImage = '/files/gallery/'; //-TODO- set different URI
		if (!$('edt_img_fll').checked) {
			theImage += 'thumbs/';
		}
		theImage += arrTmp[theImg-1].get('title')+'/'+arrTmp[theImg-1].get('text');
		...
		if (theIframe.contentWindow.document.selection)
		{
			...
			try {
				...
				theRange.pasteHTML("<img id=\"img-"+theImg+"\" src=\"" + theImage + "\" alt=\"" + theAlt + "\" class=\"" + theAlign + "\" />");
			} catch(e) {
				alert('veuillez placer le curseur dans la zone texte pour determiner l\'emplacement de l\'image');
				widgImgClose();
				return false;
			}
		...
		else
		{
			...
			theImageNode.id = 'img-'+theImg;
			theImageNode.alt = theAlt;
			theImageNode.className = theAlign;
...
function widgImgClose() {
	$('edt_img_sel').empty();
	$('tzneditor_pop_img').setStyle('display','none');
}
...
{
	var elementList = new Array("#text", "a", "em", "h1", "h2", "h3", "h4", "h5", "h6", "img", "li", "ol", "p", "strong", "ul", "table", "thead", "tbody", "tfoot", "tr", "th", "td", "div");
