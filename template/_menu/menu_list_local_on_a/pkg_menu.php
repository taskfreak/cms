...
class MenuListLocalOnA {
	...
	function MenuListLocalOnA($arrArgs) {
		$this->_startId = 1;
		$this->_levels = null;
		$this->_menuOnly = true;
	...
	* Print the menu list
	*/
	function _printMultiRecurse($activeId, $activeLine, $activeLevel, $minLevel, $objItem) {
		$go_on = true;
		$curLevel = $objItem->getOutlineLevel();
		echo '<ul id="menu'.($curLevel-1).'">';
		$i = 1;
		do {
			echo '<li><a';
			if ($curLevel == 2) {
			...
			echo ' href="'.$objItem->getUrl().'">'.$objItem->get('menu').'</a>';
